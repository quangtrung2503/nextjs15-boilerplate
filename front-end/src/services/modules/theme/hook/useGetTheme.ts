...
import themeServices, { ResponseTheme } from "../theme.services";
...
const requestAPI = themeServices.getTheme;
...
const useGetTheme = (
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseTheme>();
  const [loading, setLoading] = useState(false);
  const [refetching, setRefetching] = useState(false);
  const [error, setError] = useState<unknown>(null);

  //! Function
  const fetch: () => Promise<AxiosResponse<ResponseTheme>> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
  ...
  const checkConditionPass = useCallback((response: AxiosResponse<ResponseTheme>) => {
