...
import tagServices, { ResponseTag } from "../tag.services";
...
const requestAPI = tagServices.getTag;
...
const useGetTag = (
  ...
  const save = useSave();
  const [data, setData] = useState<ResponseTag>();
  ...
  //! Function
  const fetch: () => Promise<AxiosResponse<ResponseTag>> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
  ...
  const checkConditionPass = useCallback((response: AxiosResponse<ResponseTag>) => {
