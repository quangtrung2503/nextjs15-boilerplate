...
import bookingServices, { FiltersGetBookings, RequestGetBookings, ResponseBookingList } from "../booking.services";
...
const parseRequest = (filters: FiltersGetBookings): RequestGetBookings => {
  return cloneDeep({
    page: filters.page,
...
const requestAPI = bookingServices.getBookings;

const useGetBookings = (
  filters: FiltersGetBookings,
  ...
  //! Function
  const fetch: () => Promise<ResponseBookingList> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
