...
import statsServices, { FiltersGetStats, RequestGetStats, ResponseGetStatNewBookings } from "../stats.services";
...
const parseRequest = (filters: FiltersGetStats): RequestGetStats => {
  return cloneDeep({
    ...
    dateApplied: filters.dateApplied
  });
};

const requestAPI = statsServices.getStatNewBookings;

const useGetStatNewBookings = (
  ...
  //! Function
  const fetch: () => Promise<ResponseGetStatNewBookings> | undefined = useCallback(() => {
    if (!isTrigger) {
      return;
    }
  ...
  const checkConditionPass = useCallback((response: ResponseGetStatNewBookings) => {
...
export default useGetStatNewBookings;
